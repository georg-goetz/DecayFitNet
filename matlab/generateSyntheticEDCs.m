function edcs = generateSyntheticEDCs(T, A, noiseLevel, t, compensateULI)
if ~exist('compensateULI', 'var')
    compensateULI = true;
end
...
% Generates an EDC from the estimated parameters: this is in principal the same as decayModel(), but it is more convenient for batch processing
    ...
    for bIdx=1:batchSize
        thisDecayModel = decayModel(T(bIdx,:) , A(bIdx,:) , noiseLevel(bIdx,:) , t, compensateULI); % compensateULI
