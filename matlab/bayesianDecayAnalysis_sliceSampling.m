function [testedParameters, likelihoods] = bayesianDecayAnalysis_sliceSampling(trueEDC_db, modelOrder, tCandidates, aCandidates, nCandidates, nIterations, t)
...
assert(length(tCandidates)==length(aCandidates) && length(aCandidates)==length(nCandidates), 'There must be an equal number of T, A, and N candidates.');
nPointsPerDim = length(tCandidates);
...
y0 = rand * evaluateLikelihood(trueEDC_db, tCandidates(x0(1:modelOrder)).', aCandidates(x0(modelOrder+1:2*modelOrder)).', nCandidates(x0(2*modelOrder+1)), t);
...
for sampleIdx=1:nIterations
    ...
    while(thisX0Left(paramIdx)>1)
        ...
        thisY0Left = evaluateLikelihood(trueEDC_db, tCandidates(thisX0Left(1:modelOrder)).', aCandidates(thisX0Left(modelOrder+1:2*modelOrder)).', nCandidates(thisX0Left(2*modelOrder+1)), t);
    ...
    while(thisX0Right(paramIdx)<nPointsPerDim-1)
        ...
        thisY0Right = evaluateLikelihood(trueEDC_db, tCandidates(thisX0Right(1:modelOrder)).', aCandidates(thisX0Right(modelOrder+1:2*modelOrder)).', nCandidates(thisX0Right(2*modelOrder+1)), t);
    ...
    while(true)
        ...
        y1 = evaluateLikelihood(trueEDC_db, tCandidates(x1(1:modelOrder)).', aCandidates(x1(modelOrder+1:2*modelOrder)).', nCandidates(x1(2*modelOrder+1)), t);
