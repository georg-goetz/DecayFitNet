...
NETWORK_NAME = 'DecayFitNet_{}slopes_offset.pth'.format(N_SLOPES)
...
def get_net():
    ...
    net = core.DecayFitNetLinearExactlyNSlopes(N_SLOPES, UNITS_PER_LAYER, N_FILTER, N_LAYERS, 0, DROPOUT, 1, device)
...
def export_onnx(onnx_version=10):
    ...
    torch.onnx.export(net,  # model being run
                      ...
                      os.path.join(PATH_ONNX, f"DecayFitNet_{N_SLOPES}slopes_offset_v{onnx_version}.onnx"),  # where to save the model (can be a file or file-like object)
...
def export_input_transform2matlab(protocol=4):
    """ Helper function to dump the input transform in a different pickle protocol.
        ...
        """
    with open(os.path.join(PATH_ONNX, f'input_transform_{N_SLOPES}slopes_offset.pkl'), 'rb') as f:
        data = pickle.load(f)

    with open(os.path.join(PATH_ONNX, f'input_transform_{N_SLOPES}slopes_offset_p{protocol}.pkl'), 'wb') as f:
